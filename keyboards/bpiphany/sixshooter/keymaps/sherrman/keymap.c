...
static void tmux_paste_enter(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) {
        ...
        TMUX_STRING("P" SS_TAP(X_ENTER));
        reset_tap_dance(state);
    }
}

// Define tap dances
qk_tap_dance_action_t tap_dance_actions[] = {[SSTD_TMUX_PASTE_ENTER] = ACTION_TAP_DANCE_FN(tmux_paste_enter)};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_MEDIA] = LAYOUT
    (
        RGUI(KC_H), RGUI(KC_L), TO(_AWM_SCREEN),     \
        KC_MPRV, KC_MPLY, KC_MNXT
    ),

    /* AwesomeWM Screen Layer: AwesomeWM screen controls
    */
    [_AWM_SCREEN] = LAYOUT
    ( /* Base */
        RGUI(KC_H), RGUI(KC_L), TO(_MEDIA),     \
    ...
    */
    [_TM] = LAYOUT
    (
        SS_TMUX_PREV_WINDOW, SS_TMUX_NEXT_WINDOW, TO(_MEDIA),       \
